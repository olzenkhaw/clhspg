// ==UserScript==
// @name         PBD CLHSPG Ulasan Guru Tingkatan
// @namespace    http://tampermonkey.net/
// @version      2024-12-02
// @description  try to take over the world!
// @author       You
// @match        http://clhspg.com/*/frmENTPBSComment.aspx?*
...
function startfill()
{
    ...
    for (let i=0; i<marks.length; i++) {
        ...
        let uband = 0;
        if (marks[i] >= parseInt(document.getElementById('TP5').value)) {uband =Math.floor(Math.random() * (1 - 0 + 1) ) + 0;}
        else if (marks[i] >= parseInt(document.getElementById('TP4').value)) {uband = Math.floor(Math.random() * (3 - 2 + 1) ) + 2;}
        else {uband=Math.floor(Math.random() * (5 - 4 + 1) ) + 4;}
        var id2 = `GVSQLResult_ctl${bil}_txtComments`;
